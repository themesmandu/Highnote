...
if ( ! function_exists( 'highnote_setup' ) ) :
	...
	function highnote_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'highnote' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'highnote_custom_background_args',
				array(
...
function highnote_scripts() {
	wp_enqueue_style( 'highnote-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'highnote-style', 'rtl', 'replace' );

	wp_enqueue_script( 'highnote-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
